...
class OracledbHandlerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ["HADRON_ORACLE_USER"] = "myuser"
        os.environ["HADRON_ORACLE_PASSWORD"] = "pass"
        os.environ["HADRON_ORACLE_SID"] = "FREEPDB1"
        os.environ["HADRON_ORACLE_HOST"] = "localhost"
        os.environ["HADRON_MYSQL_TABLE"] = "sample"
    ...
    def test_handler_default(self):
        ...
        uri = "oracle://${HADRON_ORACLE_USER}:${HADRON_ORACLE_PASSWORD}@${HADRON_ORACLE_HOST}:1521/${HADRON_ORACLE_SID}"
        sb.set_persist_uri(uri)
    ...
    def test_handler_query(self):
        ...
        uri = "oracle://${HADRON_ORACLE_USER}:${HADRON_ORACLE_PASSWORD}@${HADRON_ORACLE_HOST}:1521/${HADRON_ORACLE_SID}?query=select cat, num from hadron_default"
    ...
    def test_handler_table(self):
        ...
        uri = "oracle://${HADRON_ORACLE_USER}:${HADRON_ORACLE_PASSWORD}@${HADRON_ORACLE_HOST}:1521/${HADRON_ORACLE_SID}?table=${HADRON_MYSQL_TABLE}"
        sb.set_persist_uri(uri=uri)
        sb.remove_canonical(sb.CONNECTOR_PERSIST)
        self.assertFalse(sb.pm.get_connector_handler(sb.CONNECTOR_PERSIST).exists())
        sb.save_persist_canonical(df)
    ...
    def test_handler_if_exists_global(self):
        ...
        uri = "oracle://${HADRON_ORACLE_USER}:${HADRON_ORACLE_PASSWORD}@${HADRON_ORACLE_HOST}:1521/${HADRON_ORACLE_SID}?if_exists=fail"
        sb.set_persist_uri(uri)
        sb.remove_canonical(sb.CONNECTOR_PERSIST)
        sb.save_persist_canonical(df)
    ...
    def test_connector_contract(self):
        HADRON_ADDITION = 'myAddition'
        uri = "oracle://${HADRON_ORACLE_USER}:${HADRON_ORACLE_PASSWORD}@${HADRON_ORACLE_HOST}:1521/${HADRON_ORACLE_SID}?query=select * from compare_target"
    ...
    def data(self, size: int = 10_000, complete: bool = False):
        ...
        # types
        df['cat'] = sb.tools.get_category(['SUSPENDED', 'ACTIVE', 'PENDING', 'INACTIVE'], relative_freq=[1, 99, 10, 40],
                                          size=size)
        ...
        df['date'] = sb.tools.get_datetime(start='2022-12-01', until='2023-03-31', date_format='%Y-%m-%d', ordered=True,
                                           size=size)
        ...
        if complete:
            ...
            df['date_null'] = sb.tools.get_datetime(start='2022-12-01', until='2023-03-31', date_format='%Y-%m-%d',
                                                    quantity=0.99, size=size)
            ...
            df['date_tz'] = sb.tools.get_datetime(pd.Timestamp('2021-09-01', tz='CET'),
